...
public class ProfitApiController {
    ...
    @PostMapping("/record/page/{username}")
    public PageResult<ProfitRecordInfoResponse> record(@PathVariable String username, @RequestParam(defaultValue = "1") Integer page, @RequestParam(defaultValue = "10") Integer size) {
        IPage<ProfitRecordInfoResponse> pageResult = profitService.memberRecord(username, page, size);
